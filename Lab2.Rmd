...
```{r}
dat <- read_rds("/Users/lewiswhite/MEDS/text_sentiment_analysis/text_analysis/lexisdat.RDS")
...
dat_2 <- tibble(Date = meta_df$Date,
...
```{r text_words}
text_words <- dat_2 %>%
  unnest_tokens(output = word, input = text, token = "words") #output word, take text column from data
```
...
ggplot(sent_article, aes(x = id)) +
  theme_classic() +
  geom_col(aes(y = positive), stat = "identity", fill = "slateblue") +
  geom_col(aes(y = negative), stat = "identity", fill = "red3") +
  #theme(axis.title.y = element_blank()) +
  labs(title = "sentiment Analysis: Amazon Deforestation",
       y = "Sentiment Score")
```
